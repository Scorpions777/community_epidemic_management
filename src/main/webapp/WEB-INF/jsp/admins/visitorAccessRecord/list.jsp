...
<script type="text/html" id="toolbar">
    <div class="layui-table-tool-temp" style="float: left;padding-right: 20px;">
        ...
        <input type="text" id="search_input" style="width:300px;float:right;height:30px;"
               placeholder="输入 访客姓名/身份证号/手机号 模糊查询" autocomplete="off" class="layui-input">
    </div>
</script>
...
<script>
    ...
    layui.use(['layer', 'table'], function () {
        ...
        table.render({
            ...
            url: '${pageContext.request.contextPath}/visitorAccessRecord/list', // 请求地址
            ...
            if (obj.event === 'del') {   // 点击了删除按钮
                ...
                layer.confirm('真的删除吗', function (index) {
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        data: data.field,
                        url: "/visitorAccessRecord/delete/" + data.recordId,
                            ...
                            layer.msg(
                                ...
                                function () {
                                    ...
                                    table.reload('test', {
                                        ...
                                        url: '${pageContext.request.contextPath}/visitorAccessRecord/list',  // 因为设置了分页，所以会带上参数发请求 list-data2.json?page=1&limit=5&query= （query是下面where中定义的参数名）
                                        // ,methods:"post"
                                        where: {  //设定异步数据接口的额外参数
                                            // query: inputVal
                                        },
        ...
        $('#search_input').bind('keypress', function (event) {
            ...
            if (event.keyCode == '13') {
                ...
                table.reload('test', {
                    // url可以不指定，会使用table.render中指定的
                    url: '${pageContext.request.contextPath}/visitorAccessRecord/list'  // 因为设置了分页，所以会带上参数发请求 ?page=1&limit=5&query= （query是下面where中定义的参数名）
                    ,
                    // ,methods:"post"
                    where: {  // 设定异步数据接口的额外参数
                        query: inputVal
            ...
            switch (obj.event) {
                case 'add':  // 如果是点击了增加按钮
                    layer.open({
                        ...
                        content: ["/admins/visitorAccessRecord/add"],
        ...
        table.on('edit(test)', function (obj) {
            ...
            $.ajax({
                ...
                //     name: obj.data.name,
                // },
                url: "/visitorAccessRecord/update",   // 请求地址

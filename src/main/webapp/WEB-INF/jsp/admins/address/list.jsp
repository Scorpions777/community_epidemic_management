...
<script type="text/html" id="toolbar">
    <div class="layui-table-tool-temp" style="float: left;padding-right: 20px;">
        ...
        <input type="text" id="search_input" style="width:300px;float:right;height:30px;"
               placeholder="输入 小区名称/楼号/门牌号 模糊查询" autocomplete="off" class="layui-input">
    </div>
</script>
...
<script>
    ...
    layui.use(['layer', 'table'], function () {
        ...
        table.render({
            ...
            url: '${pageContext.request.contextPath}/address/list', // 请求地址
            ...
            if (obj.event === 'del') {   // 点击了删除按钮
                ...
                layer.confirm('真的删除吗', function (index) {
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        data: data.field,
                        url: "/address/delete/" + data.addressId,
                            ...
                            layer.msg(
                                ...
                                function () {
                                    ...
                                    table.reload('test', {
                                        ...
                                        url: '${pageContext.request.contextPath}/address/list',  // 因为设置了分页，所以会带上参数发请求 list-data2.json?page=1&limit=5&query= （query是下面where中定义的参数名）
                                        // ,methods:"post"
                                        where: {  //设定异步数据接口的额外参数
                                            // query: inputVal
                                        },
        ...
        $('#search_input').bind('keypress', function (event) {
            ...
            if (event.keyCode == '13') {
                ...
                table.reload('test', {
                    // url可以不指定，会使用table.render中指定的
                    url: '${pageContext.request.contextPath}/address/list'  // 因为设置了分页，所以会带上参数发请求 ?page=1&limit=5&query= （query是下面where中定义的参数名）
                    ,
                    // ,methods:"post"
                    where: {  // 设定异步数据接口的额外参数
                        query: inputVal
